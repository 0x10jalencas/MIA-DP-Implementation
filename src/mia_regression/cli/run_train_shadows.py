...
def rf_pred_variance(model: RandomForestRegressor, X):
    """
    Find the prediction mean and variance for each sample in X using a RandomForestRegressor.

    Returns:
        pred_mean: shape (n_samples,)
        pred_var:  shape (n_samples,)
    """
...
def build_attack_features(y_true, y_pred, pred_var):
    """
    Create attack-model features from true labels, predicted labels, and prediction variance.

    Returns:
        X_attack: (n_samples, 4)
    """
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Train shadow regressors and build attack dataset."
    )
    args = ap.parse_args()
    ...
    # Train many shadow models
    for i in range(args.n_shadows):
        # Do random permutation of shadow data
        idx = rng.permutation(n)
        m_sz = int(args.shadow_train_frac * n)

        # Create member and non-member splits
    ...
    # Save attack matrix, labels, and features
    ...
    meta = {
        ...
        "saved_shadow_models": shadow_models_meta[:3]
        + (["..."] if len(shadow_models_meta) > 3 else []),
    }
